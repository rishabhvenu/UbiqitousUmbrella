module.exports.run = async utils => {
  ...
  if (!message.member.hasPermission("MANAGE_MESSAGES")) return message.channel.send(embeds.nopausepermmusic(message.author.tag));

  if (serverQueue.dispatcher.paused) return message.channel.send(embeds.alreadypausedmusic(message.author.tag));

  serverQueue.dispatcher.pause();

  message.channel.send(embeds.successpausemusic(message.author.tag));
